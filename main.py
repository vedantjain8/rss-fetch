...
def dumpp():
    ...
    if args.Key:
        ...
        for siteI in yamldef()['sites']:
            ...
            cur.execute(f"create table if not exists rss_data (id text primary key, title text not null, link text not null,summary text,author text, site text not null,image text, time text not null, pub_day text not null, pub_date text not null, pub_month text not null, pub_year text not null, pub_time text not null);")
            cur.execute(f"create trigger if not exists oldPost_delete_{site_name} before insert on rss_data when (select count(id) from rss_data where site='{site_name}') > {int(yamldef()['min_articles_to_retain_per_website'])} begin delete from rss_data where id in (select id from rss_data where site='{site_name}' order by time asc limit 1); end;")
            posts_details, post_count = rss_fetch.get_posts_details(rss=site_url)
            for post in posts_details:  
                try:
                    postTitle = cleanString(str(post["title"]))
                    insert_qry = f'insert into rss_data (id,title,link,summary,time,pub_day,pub_date,pub_month,pub_year,pub_time,author,site,image) values ("{post["id"]}", "{postTitle}","{post["link"]}","{post["summary"]}",datetime("now"),"{post["pub_day"]}" ,"{post["pub_date"]}" ,"{post["pub_month"]}","{post["pub_year"]}","{post["pub_time"]}","{post["author"]}","{site_name}","{post["image"]}");'
                    cur.execute(insert_qry)
                    #notification
                    db.commit()
                    mylog.logDef(loggLevel='info', logMSG=f">> ADDED: {site_name} [+] {postTitle}")
